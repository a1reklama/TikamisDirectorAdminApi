...
```
В случае с запросом от панели суперадмина, так:
```json
{
  "dateStart": 1706109723,
  "orderMinutes": 120,
...
- carCenters - фильтр, список автосервисов. Только для суперадмина.
...
Результат, возвращаемый от сервера (вариант ответа для панели ДИРЕКТОРА):  
```json
{
  "blocks": 
          [
...
В ответе от суперадмина будет добавлено поле `carCenter`:
```json
{
  "blocks": 
          [
...
| carCenter            | Автосервис, к которому прикреплён данный пост. Это поле будет только в варианте ответа для суперадмина      |
