...
- POST - `-api/manage/shift_calendar` - Изменить статус одного для из календаря [ссылка](#post--apimanageshift_calendar)

## Раздел АНАЛИТИКА

- POST - `-api/analytics/get-workloads` - Получить данные для экрана "Загруженность сервиса" [ссылка](#post--apianalyticsget-workloads)
- POST - `-api/order` - Добавить заказ в этом же экране. [ссылка](#post--apiorder)
...
#### GET `-api/all-works`
Получить информацию о всех услугах. Используется для наполнения фильтров и для экрана "Описание работ".  
  ...
  "works": [
    {
      ...
      "saleNote": "Работы по обнулению интервала..."
    },
    {
      "id": "ИБ-839293",
      "name": "Замена масла",
...
| saleText | string или null | Условия акции. Если акция на услугу не предусмотрена, то null. Даже если это поле null, директор/админ все равно может добавить описание. |
...
```json
{
  ...
  "mapLink": "https://go.2gis.com/7stxt",
...
| mapLink             | Ссылка на карту. Строка                                                                                                                                                                                                                                                                                                                                                                                          | 
...
#### POST `-api/manage/get-shift-calendar`

Экран "Календарь рабочих смен".  
Основной функционал экрана - это просмотр директором/суперадмином запланированного расписания постов на автосервисе  
и возможность менять количество постов.  

Тело запроса [стандартное](#общее-описание-тела-запроса-для-всех-методов-где-используется-фильтрация).  
НЕ будут игнорироваться только 2 фильтра:  
- `dateStart`
- `carCenters` (в случае с директором, он будет игнорироваться).  

Гарантируется, что в ответе от сервера будут все дни от первого числа месяца, до последнего календарного числа месяца в отсортированном порядке.   

Ответ от сервера будет такого формата:  
```json
{
  "items": [
    {...},
...
```
Цвета ячеек в фигме дизайнер напутал, поэтому могут быть несоответствия.   

| Название | Описание                                                                                                         |
|----------|------------------------------------------------------------------------------------------------------------------|
| day      | Номер дня                                                                                                        |
| capacity | Количество постов                                                                                                |
| state    | Возможны состояния ячейки (дня):                                                                                 |
...
#### POST `-api/manage/shift_calendar`

Метод вызывается, если происходит изменение в производственном календаре.  
Тело запроса в таком формате:  
```json
{
  "filters": {
...
| Поле               | Описание                                                                                                                    |
|--------------------|-----------------------------------------------------------------------------------------------------------------------------|
| filters.dateStart  | Стандартный фильтр dateStart                                                                                                |
| filters.carCenters | Для суперадмина принимается только один айди автосервиса. Для директора этот фильтр игнорируется                            |
| day                | Номер дня, которые изменяется                                                                                               |
| newCapacity        | Новое количество постов (или старое, если не изменяется)                                                                    |
| newState           | Актуальное состояние. Целое число. Возможны 2 состояния:                                                                    |
| newState 1         | НЕ выходной день. Такое состояние может быть, если директор отменяет выходной либо этот день не был установлен как выходной |     
...
Создание заказа с экрана загруженности экрана.  
Если какие-то поля я неправильно написал, или на фронте логика другая, то могу переделать детали.  
Формат запроса такой:  
```json
{
  "carCenterId": "ИБ-29388",
  "postNumber": 2,
  "unixDate": 1706109723,
  "timeInMinutes": 610,
  "phone": "+79123456789",
...
| Название      | Описание                                                                                                                                                                                                                                               |
|---------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| carCenterId   | Айди автосервиса, на котором оформляется заказ. Для Директора это поле игнорируется                                                                                                                                                                    |
| postNumber    | Номер поста, на который нужно оформить заказ                                                                                                                                                                                                           |
| unixDate      | ДАТА заказа. Это поле будет суммироваться с timeInMinutes, поэтому обязательно, чтобы у этого поля время было 00:00. Можно эти 2 поля объединить в одно или ещё как-то изменить логику, чтобы потом ошибок не возникало (смотря как на фронте сделано) |
| timeInMinutes | ВРЕМЯ заказа. Значение - это количество минут, прошедшее с полуночи. Т.е. значение "480" будет означать "08:00".                                                                                                                                       |
| phone         | телефон                                                                                                                                                                                                                                                |
